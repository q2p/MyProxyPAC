class Blake2s {
	...
	static readonly maxDigestLength = 32;
	static readonly maxKeyLength = 32;
	...
	// Hash state.
	private h = new Uint32Array(Blake2s.IV);

	// Buffer for data.
	private x = new Uint8Array(Blake2s.BLOCK_LENGTH);
	private nx = 0;
	...
	constructor(digestLength:number, key:Uint8Array, keyLength:number, salt:Uint8Array, personalization:Uint8Array) {
		if(digestLength <= 0 || digestLength > Blake2s.maxDigestLength)
			throw new Error('bad digestLength');

		this.digestLength = digestLength;

		if(keyLength < 0 || keyLength > Blake2s.maxKeyLength)
			throw new Error('key is too long');

		// XOR parts of parameter block into initial state.
		var param = new Uint8Array([digestLength & 0xff, keyLength, 1, 1]);
		this.h[0] ^= Blake2s.getInt32(param, 0);

		if(salt != null) {
			if(salt.length !== Blake2s.saltLength)
				throw new Error('salt must be ' + Blake2s.saltLength + ' bytes');

			this.h[4] ^= Blake2s.getInt32(salt, 0);
			this.h[5] ^= Blake2s.getInt32(salt, 4);
		}

		if(personalization != null) {
			if(personalization.length !== Blake2s.personalizationLength)
				throw new Error('personalization must be ' + Blake2s.personalizationLength + ' bytes');

			this.h[6] ^= Blake2s.getInt32(personalization, 0);
			this.h[7] ^= Blake2s.getInt32(personalization, 4);
		}
		...
		if(keyLength > 0) {
			...
			for(i = keyLength; i < Blake2s.BLOCK_LENGTH; i++)
				this.x[i] = 0;

			this.nx = Blake2s.BLOCK_LENGTH;
		}
	}
	
	private static getInt32(a:Uint8Array, i:number):number {
	...
	private processBlock(length:number) {
		...
		var v0  = this.h[0],
				...
				v10 = Blake2s.IV[2],
				v11 = Blake2s.IV[3],
				v12 = Blake2s.IV[4] ^ this.t0,
				v13 = Blake2s.IV[5] ^ this.t1,
				v14 = Blake2s.IV[6] ^ this.f0,
				v15 = Blake2s.IV[7] ^ this.f1;
	...
	update(data:Uint8Array):void {
		...
		if(length > left) {
			for(i = 0; i < left; i++) {
				this.x[this.nx + i] = data[offset + i];
			}
